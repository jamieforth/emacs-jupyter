...
- [[#integration-with-emacsclient][Integration with =emacsclient=]]
- [[#api][API]]
...
If code is sent to a kernel that is able to open files using the =EDITOR=
environment variable and if the =EDITOR= opens files using =emacsclient=, the
files opened are automatically associated with the corresponding REPL. This
...
sending requests to their underlying kernels that want to open files.
