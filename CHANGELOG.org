...
- If code is sent to a kernel using a REPL client, and the kernel is able to
  open files using the =EDITOR= environment variable, whose value is set to
  =emacsclient=, the files opened are automatically associated with the
  corresponding REPL. See =jupyter-server-mode-set-client=.
